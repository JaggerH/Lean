...
namespace QuantConnect.Lean.Engine.DataFeeds
{
    ...
    public class TimeSliceFactory
    {
        ...
        private readonly OrderbookDepths _emptyOrderbookDepths = new OrderbookDepths();
            ...
            Dictionary<Universe, BaseDataCollection> universeData)
        {
            ...
            Ticks ticks = null;
            OrderbookDepths orderbookDepths = null;
            Splits splits = null;
                        ...
                        if (!packet.Configuration.IsInternalFeed)
                        {
                            ...
                            switch (baseData.DataType)
                            {
                                case MarketDataType.Tick:
                                    // OrderbookDepth has DataType = Tick but is not a Tick object
                                    // Handle it separately
                                    var orderbookDepth = baseData as OrderbookDepth;
                                    if (orderbookDepth != null)
                                    {
                                        if (orderbookDepths == null)
                                        {
                                            orderbookDepths = new OrderbookDepths(algorithmTime);
                                        }
