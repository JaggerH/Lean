...
{
    /// <summary>
    /// Enhanced BrokerageRecoverySetupHandler with multi-account state recovery support.
    /// Extends the base BrokerageRecoverySetupHandler functionality to support restoring state for multi-account portfolios.
    /// </summary>
    public class BrokerageRecoverySetupHandler : ISetupHandler
    {
        ...
        /// <summary>
        /// Initializes a new BrokerageRecoverySetupHandler
        /// </summary>
        public BrokerageRecoverySetupHandler()
        {
        ...
        public IBrokerage CreateBrokerage(AlgorithmNodePacket algorithmNodePacket, IAlgorithm uninitializedAlgorithm, out IBrokerageFactory factory)
        {
            ...
            if (liveJob == null)
            {
                throw new ArgumentException("BrokerageRecoverySetupHandler.CreateBrokerage requires a live node packet");
            }
            ...
            if (accountConfigs != null && router != null)
            {
                ...
                {
                    Log.Error("BrokerageRecoverySetupHandler.CreateBrokerage(): Could not access QCAlgorithm instance, cannot replace Portfolio");
                }
            }
        ...
        public bool Setup(SetupHandlerParameters parameters)
        {
            ...
            if (liveJob == null)
            {
                AddInitializationError("BrokerageRecoverySetupHandler requires a LiveNodePacket");
                return false;
            }
        ...
        private bool LoadCashBalance(IBrokerage brokerage, IAlgorithm algorithm)
        {
            ...
            try
            {
                ...
                if (algorithm.Portfolio is MultiSecurityPortfolioManager multiPortfolio)
                {
                    ...
                    if (brokerage is PaperBrokerage paperBrokerage)
                    {
                        foreach (var accountKvp in multiPortfolio.SubAccounts)
                        {
                            ...
                            // Try to restore cash balance for this account from BrokerageData
                            var accountCashBalance = paperBrokerage.GetCashBalanceForAccount(accountName);

                            if (accountCashBalance != null && accountCashBalance.Count > 0)
                            {
                                // Zero out existing cash in this sub-account
                                ...
                                {
                                    // Set conversion rate based on currency type:
                                    // - Account currency (USD): 1.0 (permanent rate)
                                    // - Other currencies: 0 (will be updated by EnsureCurrencyDataFeeds when market data arrives)
                                    var conversionRate = (cash.Currency == subAccount.CashBook.AccountCurrency) ? 1.0m : 0m;
                    ...
                    {
                        Log.Error("BrokerageRecoverySetupHandler.LoadCashBalance(): Multi-account mode requires PaperBrokerage");
                        AddInitializationError("Multi-account mode is only supported with PaperBrokerage");
                        return false;
                    }
                }
                ...
                if (algorithm.Portfolio is MultiSecurityPortfolioManager multiPortfolio)
                {
                    ...
                    if (brokerage is PaperBrokerage paperBrokerage)
                    {
                        foreach (var accountKvp in multiPortfolio.SubAccounts)
                        {
                            ...
                            // Try to restore holdings for this account from BrokerageData
                            var accountHoldings = paperBrokerage.GetAccountHoldingsForAccount(accountName);
                            ...
                            if (accountHoldings != null && accountHoldings.Count > 0)
                            {
                                ...
                                foreach (var holding in accountHoldings.OrderByDescending(x => x.Type))
                                {
                    ...
                    {
                        Log.Error("BrokerageRecoverySetupHandler.LoadExistingHoldingsAndOrders(): Multi-account mode requires PaperBrokerage");
                        AddInitializationError("Multi-account mode is only supported with PaperBrokerage");
                        return false;
                    }
                }
        ...
        protected void GetOpenOrders(IAlgorithm algorithm, IResultHandler resultHandler, ITransactionHandler transactionHandler, IBrokerage brokerage)
        {
            ...
            foreach (var order in openOrders.OrderByDescending(x => x.SecurityType))
            {
                ...
                resultHandler.DebugMessage($"BrokerageRecoverySetupHandler.Setup(): Open order detected.  Creating order tickets for open order {order.Symbol.Value} with quantity {order.Quantity}. Beware that this order ticket may not accurately reflect the quantity of the order if the open order is partially filled.");
            }
        }
        ...
        private IOrderRouter CreateRouterFromConfig(MultiAccountConfig config)
        {
            ...
            switch (routerType)
            {
                ...
                case "symbol":
                    Log.Error("BrokerageRecoverySetupHandler: Symbol-based routing not fully implemented, falling back to MarketBasedRouter");
                    return new MarketBasedRouter(config.Router.Mappings, defaultAccount);

                default:
                    Log.Error($"BrokerageRecoverySetupHandler: Unknown router type '{routerType}', using MarketBasedRouter");
                    return new MarketBasedRouter(config.Router.Mappings, defaultAccount);
            }
        }
        ...
        private Dictionary<SecurityType, string> ParseSecurityTypeMappings(Dictionary<string, string> mappings)
        {
            ...
            foreach (var kvp in mappings)
            {
                ...
                {
                    Log.Error($"BrokerageRecoverySetupHandler: Invalid SecurityType '{kvp.Key}' in router mappings");
                }
            }
