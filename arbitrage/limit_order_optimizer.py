...
class LimitOrderOptimizer:
    ...
    @staticmethod
    def calculate_buy_limit_price(best_bid: float, best_ask: float) -> float:
        """
        计算买入限价单价格
        ...
        """
        aggression = 0.6  # Fixed aggression level
        spread = best_ask - best_bid
        return best_bid + spread * aggression

    @staticmethod
    def calculate_sell_limit_price(best_bid: float, best_ask: float) -> float:
        """
        计算卖出限价单价格
