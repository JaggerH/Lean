"""
TradingPair + ArbitrageAlphaModel Framework Integration Test - Crypto Future (Auto-Configuration)

Test Scenario:
- Data Source: Gate (crypto spot) + Gate (crypto futures)
- Trading Pairs: Gate pepeusdt (spot) / PEPEUSDT (future), Gate dogeusdt (spot) / DOGEUSDT (future)
- Date Range: 2025-10-01 to 2025-10-31
- Account Config:
  * Gate Account: $100,000 - trades Gate crypto spot and futures
- Test Focus: Grid level auto-configuration via ArbitrageAlphaModel for spot/future pairs
...
1. Verify ArbitrageAlphaModel auto-configures grid levels for "spot_future" pairs
...
8. Verify single-account routing (Gate crypto spot and futures -> Gate)
...
- AddPair(leg1, leg2, "spot_future") triggers auto-configuration
- Default "spot_future" template adds 2 grid levels:
  * LONG_SPREAD: entry=-1.5%, exit=+0.8%, size=30%
  * SHORT_SPREAD: entry=+2.5%, exit=-0.8%, size=30%
...
class CryptoFutureFrameworkTest(AQCAlgorithm):
    """TradingPair + ArbitrageAlphaModel Framework Integration Test - Crypto Future"""
    ...
    def Initialize(self):
        ...
        # === 3. Add securities (Gate crypto spot + crypto futures) ===
        self.Debug("=" * 60)
        self.Debug("INITIALIZING SECURITIES")
        self.Debug("=" * 60)

        # Add Gate crypto spot (lowercase symbols)
        ...
        # Using Resolution.Tick for spot crypto
        self.pepe_spot = self.AddCrypto("pepeusdt", Resolution.ORDERBOOK, Market.Gate)
        self.doge_spot = self.AddCrypto("dogeusdt", Resolution.ORDERBOOK, Market.Gate)

        # Add Gate crypto futures (uppercase symbols)
        self.pepe_future = self.AddCryptoFuture("PEPEUSDT", Resolution.ORDERBOOK, Market.Gate)
        self.doge_future = self.AddCryptoFuture("DOGEUSDT", Resolution.ORDERBOOK, Market.Gate)

        self.Debug(f"Added {len(self.Securities)} securities")
        self.Debug(f"  Gate crypto spot securities will use GateBrokerageModel (leverage, fees, etc.)")
        self.Debug(f"  Gate crypto future securities will use GateFuturesBrokerageModel")
        ...
        # AddPair with pairType "spot_future" triggers auto-configuration
        pepe_pair = self.TradingPairs.AddPair(
            self.pepe_spot.Symbol,
            self.pepe_future.Symbol,
            "spot_future"  # This triggers auto-configuration!
        )
        self.Debug(f"Added PEPE pair (type: spot_future) - grid levels auto-configured")

        doge_pair = self.TradingPairs.AddPair(
            self.doge_spot.Symbol,
            self.doge_future.Symbol,
            "spot_future"  # This triggers auto-configuration!
        )
        self.Debug(f"Added DOGE pair (type: spot_future) - grid levels auto-configured")

        self.Debug(f"Added {self.TradingPairs.Count} trading pairs")
        self.Debug(f"PEPE grid levels (auto-configured): {len(list(pepe_pair.LevelPairs))}")
        self.Debug(f"DOGE grid levels (auto-configured): {len(list(doge_pair.LevelPairs))}")
        self.Debug("=" * 60)
    ...
    def OnEndOfAlgorithm(self):
        ...
        self.Debug("")
        self.Debug("=" * 60)
        self.Debug("TEST RESULTS - Crypto Future Framework Integration Test")
        self.Debug("=" * 60)
